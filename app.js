...
const EXTRA_WORD_POOL = EXTRA_WORD_POOL_RAW
  .split(/\r?\n/)
  .map((word) => word.trim().toLowerCase())
  .map((word) => word.normalize("NFD").replace(/[\u0300-\u036f]/g, ""))
  .filter((word) => /^[a-z][a-z'-]*$/.test(word));
    ...
    for (let i = 0; i < targetLength; i += 1) {
      const candidates = availableWords
        .map((w) => {
          ...
          return { w, overlap, subCost };
        })
        .filter((c) => c.overlap > 0 || c.subCost <= 1)
        .sort((a, b) => (b.overlap - a.overlap) || (a.subCost - b.subCost));

      const selectionPool = candidates.slice(0, 25);
      const pick = randomChoice(selectionPool.length ? selectionPool : candidates);
      if (!pick) break;

      phrase.push(pick.w);

      for (const ch of pick.w) {
